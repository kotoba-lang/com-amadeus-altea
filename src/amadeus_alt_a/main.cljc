(ns amadeus_alt_a.main
  "Kotodama WASM entrypoint for the Amadeus_altéa clean-room actor (L4) — Clojure port.
  ...
  facts (`amadeus_alt_a.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "amadeus_alt_a")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"amadeus_alt_a.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "amadeus_altéa-compat" :tier tier :entities entities} 200])
...
(defn start! [] :amadeus_altéa-compat/ready)
