(ns amadeus_alt_a.main-test
  "Contract + behavioral test for the amadeus_alt_a L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "amadeus_altéa-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'amadeus_alt_a.main-test)]
